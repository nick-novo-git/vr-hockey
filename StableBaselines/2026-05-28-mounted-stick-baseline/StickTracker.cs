    ...
    public sealed class StickTracker : MonoBehaviour
    {
            ...
            Vector3 offsetControllerPosition)
        {
            ...
            if (isAutoCalibrating)
            {
                ...
                if (Time.time >= calibrationEndsAt && calibrationSampleCount >= minimumCalibrationSamples)
                {
        ...
        private void SampleMountedRestPose(Vector3 controllerPosition, Quaternion controllerRotation)
        {
            ...
            if (calibrationSampleCount > 0 &&
                (Vector3.Distance(controllerPosition, lastCalibrationSamplePosition) > maxCalibrationSampleMovement ||
                 Quaternion.Angle(controllerRotation, lastCalibrationSampleRotation) > maxCalibrationSampleAngle))
            {
                return;
            }
